...
MongoDB is optional for the demo. If `MONGO_URI` is absent or unavailable, the API uses seeded in memory demo data so the product still works.
...
Set `VITE_GOOGLE_MAPS_API_KEY` to use Google Maps. Without it, Foodcane automatically uses Leaflet and OpenStreetMap.
